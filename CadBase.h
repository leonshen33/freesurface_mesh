...
public:
	...
	static CMatrix3D CreateRotateMatrix(double da,Node bv);
	static CMatrix3D CreateScaleMatrix(double);
	static CMatrix3D CreateTransfMatrix(VECTOR3D vec);
	static CMatrix3D CreateTransfMatrix(Node vec);
...
public:
	...
	unsigned int GetRelationWith(BOX2D b) const;
protected:
	void normalize();
};
...
// get attribs
	...
	enum {enumSeparated,enumIntersected};
	unsigned int GetRelationWith(BOX3D b) const;
